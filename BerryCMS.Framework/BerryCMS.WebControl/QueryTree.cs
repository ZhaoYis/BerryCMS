    ...
    public static class QueryTree
    {
        ...
        public static List<T> TreeWhere<T>(this List<T> entityList, Predicate<T> condition, string primaryKey, string parentId = "ParentId") where T : class
        {
            ...
            foreach (T entity in locateList)
            {
                ...
                treeList.Add(entity);
                //向上查询
                PropertyInfo propertyInfo = entity.GetType().GetProperty(parentId);
                if (propertyInfo != null)
                {
                    ...
                    while (true)
                    {
                        ...
                        if (upRecord != null)
                        {
                            ...
                            PropertyInfo data = upRecord.GetType().GetProperty(parentId);

                            pId = data != null ? data.GetValue(upRecord, null).ToString() : "";
                        }
                        else
                        {
                            break;
